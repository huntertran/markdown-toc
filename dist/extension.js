...
class RegexStrings {
    constructor() {
        ...
        this.REGEXP_ANCHOR = /\[.+\]\(#(.+)\)/;
        this.REGEXP_IGNORE_TITLE = /<!--\s*TOC\s+ingore:true\s*-->/si;
...
class HeaderManager {
    ...
    async getDocumentSymbols(fileUri) {
        ...
        return allSymbols.filter(function (sym) {
            return sym.kind === vscode_1.SymbolKind.String;
        });
    }
    async getHeaderList() {
        let headerList = [];
...
class Header {
    constructor(anchorMode, preserveUnicodeAnchors = false) {
        ...
        this.anchor = new Anchor_1.Anchor("", preserveUnicodeAnchors);
    }
    convertFromSymbol(symbol) {
        ...
        this.anchor = new Anchor_1.Anchor(this.cleanUpTitle(this.dirtyTitle), this.preserveUnicodeAnchors);
    }
    get depth() {
    ...
    tocRowWithAnchor(tocString) {
        ...
        let ANCHOR_MARKDOWN_HEADER = __webpack_require__(13);
...
const AnchorEncoder_1 = __webpack_require__(12);
class Anchor {
    constructor(headerText, preserveUnicodeAnchors = false) {
        this.id = "";
        this.name = "";
        this.range = new vscode_1.Range(0, 0, 0, 0);
        headerText = headerText.toLowerCase().replace(/\s/gi, "-");
        let encodedHeaderText = preserveUnicodeAnchors ? (0, AnchorEncoder_1.encodeURIComponentPreservingUnicode)(headerText) : encodeURIComponent(headerText);
...
exports.encodeURIComponentPreservingUnicode = encodeURIComponentPreservingUnicode;
...
function encodeURIComponentPreservingUnicode(value) {
...
function activate(context) {
    ...
    // Events
    let saveMarkdownToc = vscode_1.workspace.onDidSaveTextDocument(() => {
