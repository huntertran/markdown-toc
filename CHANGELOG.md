...
- [#41](https://github.com/huntertran/markdown-toc/issues/41): a `<!-- TOC ignore:true -->` above the TOC block cost the TOC its own options - `<!-- TOC depthFrom:2 depthTo:3 updateOnSave:false -->` was rewritten as a bare `<!-- TOC -->` on the next Insert/Update. The ignore marker also begins with `TOC`, so the option reader stopped at it and never reached the real start marker. Ignore markers are now skipped when the options are read, the same way the TOC block scan already skipped them
...
- Options set in one document's `<!-- TOC ... -->` marker kept applying to every other document for the rest of the session. (This release note originally credited [#41](https://github.com/huntertran/markdown-toc/issues/41) with it; that is a different bug, fixed in 3.1.2)
